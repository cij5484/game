...
**Current Source of Truth — 대규모 Horde 성능 / M9 Prototype Technical Requirement**

작성일: 2026-09-18. **목표 설계 문서이며 구현 완료 보고가 아니다.**
기준: [GDD v0.12](GAME_GDD_v0.12.md) 전체 게임 설계·Prototype 기본값·개발 도구·Future·역사 기록을 승계한다. v0.12 원본은 수정하지 않는 historical record다. M8 `1719e74694e4cdf10211f3d04d1c976221223032`를 [PR23](https://github.com/cij5484/game/pull/23) / main `7d1cae9`로 병합한 뒤 `codex/prototype-m9-horde-performance`에서 진행한다. 최신 구현/검증은 [M9 기록](../prototype-m9-horde-performance.md)을 따른다.

M9는 새 콘텐츠나 Gameplay Balance 변경 없이 대규모 Horde의 중복 Rendering·Targeting·State allocation을 줄인다. 최신 기술 요구사항은 **§1.24 / §2.9 / §3.11 / §5**, 승계한 개발 도구는 §1.23·§2.8·§3.10, 게임 규칙은 §1·§2.7·§3.9를 따른다. M8과 이전 검증은 역사 기록이며 M9 검증으로 재사용하지 않는다. `/dev` 패널은 **Prototype Development Tool / Not Final Game Feature**이며 Runtime Override는 GDD 확정값이나 정식 Default 변경이 아니다. M9는 **commit/push 후 정지**, main merge와 다음 Milestone 자동 시작은 하지 않는다.
...
Prototype은 물량 공세, 대량 제거 쾌감, 강해져도 유지되는 긴장, **한 Run 안의 체감 성장**을 검증한다. 위의 Run 밖 성장 목표는 유지하지만 완성형 Meta·Stage 2+·신규 캐릭터·Challenge/Endless는 현재 범위 밖이다. 구현 범위는 §2, 시험 수치는 §3, 보류 설계는 §4, 현재 코드와의 차이는 §5를 따른다.
...
아래는 **첫 시험 목표이며 최종 확정 수치가 아니다.** Future Economy 숫자는 설계 기록이며 1차 Prototype 구현 의무가 아니다. 장기/Future 수치를 현재 데이터나 테스트 기대값으로 자동 적용하지 않는다. M3 화면 규칙은 유지하며 M4/M5 당시 Tuning은 역사 기록이다. 최신 변경 수치는 §3.9 M7을 따르며 나머지 M6 수치는 유지한다.
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. v0.4/v0.5를 포함한 과거 버전은 역사 문서로 보존하며 상충 시 최신 v0.13을 우선한다.
