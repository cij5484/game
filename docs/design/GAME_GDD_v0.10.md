...
**Current Source of Truth — Prototype High-roll / Combat Tempo / M6**

작성일: 2026-09-18. **목표 설계 문서이며 구현 완료 보고가 아니다.**
기준: [GDD v0.9](GAME_GDD_v0.9.md) 전체를 승계하고 M6 사용자 명세와 직접 Playtest 피드백을 반영한다. v0.9 원본은 수정하지 않는 historical record다. M5 `0ca568c7b0296dc123b99cc3171fe5b5de8beb22`는 PR20 / main `4bee754`로 병합했다. 최신 구현/수치/검증은 [M6 기록](../prototype-m6-highroll-tempo.md)을 따른다.

상충 시 v0.10의 **§2.6 / §3.8 / §5 최신 M6 상태**를 우선한다. §1의 전체10 Relic/5 Core/6 Synergy와 기존 미래 설계를 보존한다. 아래 M3/M4/M5라고 명시한 범위·수치·검증은 당시 기록이며 그 절의 “현재/이번/Future”는 해당 Milestone 기준이다. M6은 정상 전투 Tempo, 생성 HP/XP/수류탄 조정과6 Relic/3 Core/3 자동 Synergy를 구현한다. Boss/Meta/나머지 전체 Pool은 이번 범위 밖이다. M6 branch는 commit/push 후 멈추고 main merge/다음 Milestone은 하지 않는다.
...
M3 당시 두 슬롯은 Locked였으며 실제 계정 해금/특수무기는 없었다. M5 개발 테스트에서는 두 슬롯을 Unlocked/Empty로 시작하고 Lv5/10 획득과 실제 무기를 연결한다. 향후 무장 확장 Core의 2→3 슬롯은 동적 추가 가능하게 하되 Core/세 번째 슬롯 획득은 Future다.
...
### 2.6 M6 Prototype Scope — 최신 구현 범위
...
아래는 **첫 시험 목표이며 최종 확정 수치가 아니다.** Future Economy 숫자는 설계 기록이며 1차 Prototype 구현 의무가 아니다. 장기/Future 수치를 현재 데이터나 테스트 기대값으로 자동 적용하지 않는다. M3 화면 규칙은 유지하며 M4/M5 당시 Tuning은 역사 기록이다. 최신 수치는 §3.8 M6을 따른다.
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. v0.4/v0.5를 포함한 과거 버전은 역사 문서로 보존하며 상충 시 최신 v0.10을 우선한다.
