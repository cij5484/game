...
**Current Source of Truth — Prototype Scope / Stage 1 Gameplay Loop**

작성일: 2026-09-18. **목표 설계 문서이며 구현 완료 보고가 아니다.**
기준: [GDD v0.5](GAME_GDD_v0.5.md) 전체와 이후 사용자 확정 사항. v0.5는 수정하지 않는 historical design record다. 현재 gameplay implementation baseline은 main에 merge된 `0489ef9cdfcf39870e53178f82e531d8bac4931d`이며, 이번 문서는 구현 상태를 변경하지 않는다.

이 문서를 향후 설계 판단에 우선한다. v0.1~v0.5, Technical Spec, 과거 계획·완료 기록은 당시의 설계와 구현 기록이다. 이번 패스는 문서만 변경하며 gameplay, balance data, UI, Asset, 테스트 수치를 변경하지 않는다.
...
약점은 **강제 면역 Phase가 아니다**. 준비 중 본체를 압도적 화력으로 먼저 죽일 수 있다. 약점 위치·요구 피해·HP 전환 임계·Telegraph 시간·공격값은 Tuning/미정이다. Boss전 약1분 전후부터 시험한다.
...
**아래는 향후 1차 Prototype에서 구현할 범위이지 이번 문서 작업의 구현 완료 목록이 아니다.** 장기 전체 Pool과 실제 첫 구현 Pool을 구분한다. Marine 고정, Stage 1 하나로 핵심 재미를 검증한다.
...
| 특수무기 | 수류탄=공간/Horde, 미사일=위험 Target, 드론=지속 플랫폼 3종. 기본 최대2종과 Character Lv5/10 획득 구조를 해금 테스트 상태로 검증 |
...
| --- | --- | --- |
| Lv15 초월 | 무기별 큰 후보 Pool 방향 | 무기당 약3개 실제 후보. 구체 선정 미정 |
| Lv20 Overclock | 무기당 정확히5종 목표 Pool | 약3종씩 우선 구현. 구체 선정 미정 |
...
아래는 **첫 시험 목표이며 최종 확정 수치가 아니다.** Future Economy 숫자는 설계 기록이며 1차 Prototype 구현 의무가 아니다. 데이터 파일이나 테스트 기대값에 이번 패스로 적용하지 않는다.
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. v0.4/v0.5는 역사 문서로 보존하며 상충 시 v0.6을 우선한다.

## 5. Current Implementation Gap

gameplay baseline은 main에 merge된 `0489ef9cdfcf39870e53178f82e531d8bac4931d`다. v0.5까지의 코드 확인 기록을 계승한다. 이번 문서 작업에서 gameplay 구현·테스트/build·플레이 검증은 하지 않았다. 과거 verification을 최신 결과로 바꾸지 않는다. 코드 경로는 `src/game/` 기준이다.

| 구분 | 영역 | 현재 상태 → Prototype 목표 |
| --- | --- | --- |
| 현재 존재 | 전장/입력 | Portrait/full-bleed·Wall·3 Soft Lane·progress01/perspective·자동 공격·Tap 집중/빈 Tap 복귀 존재. 유지하며 새 범위 통합 (`battlefield/`, `combat/targeting.ts`) |
| 기존 수정 | Range/초기 공격 | 현재200ms 피해10·Grunt HP30, 사거리 필터 없는 타겟팅 → Marine 중거리 밖 발사 금지·느린 단발·초기Grunt1-shot tuning (`data/weapons.ts`, `data/enemies.ts`) |
| 기존 수정/신규 | 기본 개조6종 | 현재 관통/도탄/다중탄/폭발탄/처형탄, MAX5/Lv3 A/B → 점사·고위력 추가, 처형 제외, 기본3종·반복 성장·역할 차이 (`data/traits.ts`) |
| 현재 존재/기존 수정 | XP/선택 | XP·Level·기본3카드 존재. 공용 강화 MAX5·유한Pool → 반복 성장/유효 후보 유지 (`progression/progression.ts`, `data/upgrades.ts`) |
| 기존 수정/신규 | Rarity/가중치/대성공 | Lv3/5 희귀도 매핑·기존 투자bias1.6/특성2가중치 → 희귀도 독립·새 투자 가중치·Great Success·마일스톤 Queue |
| 신규 | 특수무기3종 | 현재 Marine은 Magic2종 사용 → 수류탄/미사일/드론 실제 전투 역할 및 최대2종 획득. Magic 콘텐츠는 미래 Mage용 보존 |
| 신규 | 특수 성장 | Lv3/6/10/15/20 전용 구조 없음 → 전체 성장 구조와 무기당 초월/Overclock 약3개 대표 후보, 선택 누락 없는 Queue |
| 기존 수정 | 일반 적 | Grunt/Runner/Shield는 존재 → Stage1 역할/비율, Shield 단순 피해배율 대신 방패HP→본체HP (`enemies/enemySimulation.ts`, `data/enemies.ts`) |
| 기존 수정/신규 | Elite2종 | 기존 Elite 기반은 있으나 광폭 돌진 준비/Telegraph/급가속, 중장 방패 부분 보호 미구현 |
| 기존 수정 | Stage1 Director | 현재300초·13단계·초기48·cap72→180 → 약20분 Pressure/Relaxation, Lane별 조합 변화·후반 폭증. 기존 숫자 자동 승계 안 함 (`data/run.ts`, `data/horde.ts`) |
| 신규 | 공성 거인 | 접근/공성 준비/약점/Stagger/증원/최후 돌진·Boss 종료 전투 미구현 |
| 기존 수정 | Relic 부분 Pool | Legacy8종·Lv1~5·기본3개 → 무레벨·무패널티·기본2·교체/포기·대표5~6종 (`data/relics.ts`) |
| 기존 수정/신규 | Core 부분 Pool | 기존4종·최대2·Elite8% → 대표3종·Run1·유효Pool 즉시 지급·품질 소급·획득 후 Roll 종료 (`data/cores.ts`) |
| 기존 수정/신규 | Synergy 부분 Pool | 기존5종·해금 후 카드 선택/후보 보장 → 기본Build+특수A/B Lv10 3중 조건 자동 활성, 대표3종 (`data/synergies.ts`) |
| 기존 수정 | Elite Reward Rhythm | 기존 보상 경로를 약5 Encounter·첫 유물 기회/이후30~35%·Core3% 첫안과 비교하여 조정. 미드롭 처치 보상도 필요 |
| Future, 현재 미구현 | Boss Meta Reward | Gold/Credits/첫Clear/기록/희귀재화/해금 방향만 기록. 실제 지급·저장·계정 경제는 현재 범위 밖 |
| Future, 현재 미구현 | Gold/Credits/기록 | Gold20/10/5 연구·강한Meta·Credits·작전 기록/Point·점진 해금UX·Reroll Meta·희귀 재화 보류 |
| Future, 현재 미구현 | 모드/캐릭터 | Challenge·Endless·Stage2+·신규Character·Awakening·Variant·Preset·Cosmetic·친구기록/Ranking 보류 |
| Future/후속 범위 | 전체 콘텐츠/상세 UI | 전체Relic10/Core5/Synergy6·모든Lv15/20·완성Tutorial·장기Header/Build UI·Run 사거리 카드 상세 성장 등은 필수 범위와 구분 |

기존 스팀팩·V 필살기·Pause/재시작 등 존재 기능은 이번 문서로 구현 완료 상태를 바꾸지 않는다. 현 치명 피해1.75배 등 gameplay/balance data와 테스트 기대값은 변경하지 않는다. 미래 기능이 Gap에 있다고 자동 구현할 수 없다.

### 5.1 v0.6 적용 경계

v0.5 전체 방향을 폐기하지 않고 Stage1 Enemy/Elite/Boss·20분 리듬·보상·경제/기록·Future Character/Awakening을 구체화했다. Stage2 구체 배치는 승계하지 않는다. Prototype Scope와 전체 설계 Pool을 분리한다. 문서 commit/push까지만 진행하며 main merge와 다음 gameplay 구현은 별도 요청 없이 시작하지 않는다.
