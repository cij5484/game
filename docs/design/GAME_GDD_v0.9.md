...
**Current Source of Truth — Marine Special Weapons / M5**

작성일: 2026-09-18. **목표 설계 문서이며 구현 완료 보고가 아니다.**
기준: [GDD v0.8](GAME_GDD_v0.8.md) 전체와 M5 사용자 명세 및 추가 개발용 배속/생성 HP 요청. v0.8 원본은 수정하지 않는 historical record다. M4는 [PR #19](https://github.com/cij5484/game/pull/19) / main `68b9922`에 merge됐다. M5의 실제 구현/수치/검증 상태는 [구현 기록](../prototype-m5-special-weapons.md)을 따른다.

상충 시 v0.9를 우선한다. v0.1~v0.8, Technical Spec, 과거 계획은 역사적 기록이다. 이번 M5는 특수무기3종·독립 Level·Lv3/6/10/15/20·특별 선택 Queue·하단 Owner HUD를 구현한다. 아래 M3/M4라고 명시한 항목은 당시 구현 기록이며 최신 상태는 §2.5/§3.7/§5가 우선한다. 새 유물/코어/명명 시너지/Boss/Meta는 이번 범위에 포함하지 않는다.
...
### 2.5 M5 Prototype Scope — 실제 구현 경계
...
아래는 **첫 시험 목표이며 최종 확정 수치가 아니다.** Future Economy 숫자는 설계 기록이며 1차 Prototype 구현 의무가 아니다. 장기/Future 수치를 현재 데이터나 테스트 기대값으로 자동 적용하지 않는다. M3 화면 규칙과 M4 Tuning은 유지하고 M5 추가값은 §3.7을 따른다.
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. v0.4/v0.5를 포함한 과거 버전은 역사 문서로 보존하며 상충 시 최신 v0.9를 우선한다.
