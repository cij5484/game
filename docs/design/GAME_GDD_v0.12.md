...
**Current Source of Truth — 한국어 실시간 개발자 밸런스 패널 / M8**

작성일: 2026-09-18. **목표 설계 문서이며 구현 완료 보고가 아니다.**
기준: [GDD v0.11](GAME_GDD_v0.11.md) 전체 게임 설계·Prototype 기본값·Future·역사 기록을 승계한다. v0.11 원본은 수정하지 않는 historical record다. M7 `60f19c02dceceaa1009562f866024632f5b29e8d`을 [PR22](https://github.com/cij5484/game/pull/22) / main `9fa09f4`로 병합한 뒤 `codex/prototype-m8-dev-balance-panel`에서 진행한다. 최신 구현/검증은 [M8 기록](../prototype-m8-dev-balance-panel.md)을 따른다.

M8은 **Prototype Development Tool / Not Final Game Feature**인 별도 `/dev` 패널을 추가한다. M7 게임 콘텐츠와 코드 기본 밸런스는 변경하지 않는다. 최신 개발 도구 규칙은 **§1.23 / §2.8 / §3.10 / §5**를 우선하고 게임 규칙은 승계한 §1·§2.7·§3.9를 따른다. M3~M7 또는 Historical로 명시한 당시 검증을 M8 검증으로 재사용하지 않는다. 패널의 Runtime Override는 GDD 확정값이나 정식 Default 변경이 아니며 사용자가 별도로 확정한 뒤 반영한다. M8은 **commit/push 후 정지**, main merge와 다음 Milestone 자동 시작은 하지 않는다.
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. v0.4/v0.5를 포함한 과거 버전은 역사 문서로 보존하며 상충 시 최신 v0.12를 우선한다.
