...
**Current Source of Truth — Horde Pressure / Marine Weapon Growth**

작성일: 2026-09-18. **목표 설계 문서이며 구현 완료 보고가 아니다.**
기준: [GDD v0.7](GAME_GDD_v0.7.md) 전체와 M4 사용자 명세 및 추가 플레이 피드백. v0.7은 수정하지 않는 historical record다. M3 `0f408ddef26ca3c36bd9d128f4e6230edf5b33cd`는 PR #18 / main `b86b6915e5aea4a171f74dc7229a97516a0e2c11`에 merge됐다. M4 실제 구현/수치/검증은 [구현 기록](../prototype-m4-horde-weapon-growth.md)을 따른다.

상충 시 v0.8을 우선한다. v0.1~v0.7, Technical Spec, 과거 계획은 역사적 기록이다. 이번 M4는 적 수/보충량과 Marine의 공용3종·개조6종·사거리·희귀도·대성공 및 기존 HUD 연결을 구현한다. 아래 M3라고 명시한 항목은 당시 구현 기록이며 최신 상태는 §3.6/§5가 우선한다. 특수무기 이후 시스템은 구현하지 않는다.
...
아래는 **첫 시험 목표이며 최종 확정 수치가 아니다.** Future Economy 숫자는 설계 기록이며 1차 Prototype 구현 의무가 아니다. 장기/Future 수치를 현재 데이터나 테스트 기대값으로 자동 적용하지 않는다. M3 화면 규칙은 유지하고 최신 M4 Tuning은 §3.6을 따른다.
...
성능 첫 방향은 일반 **1배**, 희귀 **약 1.6배**, 유니크 **약 2.4배**, 전설 **약 3배 이상 또는 행동 변화**다. 공격력·공속·치명 확률에 기계적으로 같은 배율을 적용하지 않는다. M4 공용/기본무기/사거리의 첫안 효과와 점감은 구현 기록을 따른다. 미구현 특수무기 coefficient와 최종 밸런스는 후속이다.
...
### 3.6 M4 — Horde + Weapon Growth, 현재 Prototype Tuning
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. v0.4/v0.5는 역사 문서로 보존하며 상충 시 최신 v0.8을 우선한다.

## 5. Current Implementation Gap

현재 gameplay 기반: M1 `d15b41d` → M2 `a656b12` → M3 `0f408dd` (PR18 main `b86b691`). M4는 새 브랜치의 물량/성장 구현이며 main에는 아직 merge하지 않는다. 검증은 [M4 구현 기록](../prototype-m4-horde-weapon-growth.md), 역사 기록은 [M1](../prototype-m1-combat-foundation.md)/[M2](../prototype-m2-stage1-core.md)/[M3](../prototype-m3-layout-build-hud.md)를 따른다. 역사적 `0489ef9` 검증 결과를 최신 결과로 바꾸지 않는다.

| 영역 | 현재 실제 상태 / 남은 Gap |
| --- | --- |
| M1 기본 전투 | Range .55,800ms 단발 피해10. M4 성장으로 Range/Cycle/행동 변경 가능 |
| M2 적/Director | HP/물리 방패/두Elite 행동/20분 임시 종료 유지. M4 count/batch/cap 상승, M3 speed1.5배 그대로 |
...
| 특수무기 | 잠긴2슬롯만 존재. Grenade/Missile/Drone,획득/트리/Lv10·15·20/마일스톤Queue 미구현 |
...
| 성능/표현 | 자동20분simulation은 렌더링stub. 실기기 후반최대700대상 성능/입력감/밀도 최종평가 미실시. Final Art/완성Tutorial 미구현 |
...
- M4는 별도 branch commit/push 후 STOP. main merge나 다음 구현은 별도 사용자 요청을 따른다.
