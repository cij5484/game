...
**Current Source of Truth — Random Acquisition / Basic Mod Categories / Stage 1 Boss / M7**

작성일: 2026-09-18. **목표 설계 문서이며 구현 완료 보고가 아니다.**
기준: [GDD v0.10](GAME_GDD_v0.10.md) 전체 설계·Future·역사 기록을 승계하고 M7 사용자 명세와 직접 Playtest 피드백을 반영한다. v0.10 원본은 수정하지 않는 historical record다. M6 `4b06c39e82683cd809fbb2bec544424cd5aa1187`을 [PR21](https://github.com/cij5484/game/pull/21) / main `544aa1f`로 먼저 병합한 뒤 `codex/prototype-m7-boss-acquisition`에서 진행한다. 최신 구현/수치/검증은 [M7 기록](../prototype-m7-boss-acquisition.md)을 따른다.

상충 시 v0.11의 **§1 획득·후보·Boss 규칙 / §2.7 / §3.9 / §5 최신 M7 상태**를 우선한다. §1의 전체10 Relic/5 Core/6 Synergy와 미래 설계는 보존한다. M3/M4/M5/M6 또는 Historical이라고 명시한 범위·수치·검증과 그 절의 “현재/이번/Future”는 당시 기록이다. 특히 역사 절의 Lv5/10 고정 지급·20분 임시 종료는 M7에서 폐기했다. M7은 공성 거인까지 구현하며 Meta/나머지 전체 Pool은 범위 밖이다. M7 branch는 **commit/push 후 정지**, main merge/다음 Milestone 자동 시작은 하지 않는다.
...
### 2.7 M7 Prototype Scope — 최신 구현 범위
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. v0.4/v0.5를 포함한 과거 버전은 역사 문서로 보존하며 상충 시 최신 v0.11을 우선한다.
