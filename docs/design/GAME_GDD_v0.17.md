...
**Current Source of Truth — M12 Mastery / Operation Records / 실제 점진 해금**
...
| --- | --- | --- | --- | --- |
| 공성 삼각타격 | 관통 + 소이탄 | 벙커 버스터 | 킬 체인 | 강적에 공성 표식 → 세 무기 집중 → 장갑 붕괴와 큰 추가 피해 |
...
| 추적 섬멸망 | 점사 + 소이탄 | 전술 사냥꾼 | 울프팩 | 위험 적에 사냥 표식 → 세 무기 집중 → 처치 후 다음 위험 적으로 연쇄 |
| 역추적 방공망 | 도탄 + 소이탄 | 불사조 미사일 | 전술 윙맨 | 원거리 적 공격 시 발사 위치 노출 → 미사일·드론·도탄 역추적 및 강화 공격 |

명칭 대응: 개틀링 건십 = Gatling Gunship, 울프팩 = Wolfpack, 전술 윙맨 = Tactical Wingman 계열, ‘소이탄’ = 소이탄. 기본무기 개조의 요구 Level, 표식·노드·장악 판정, 배율·지속시간·cooldown은 미정/Prototype Tuning이다. 여기의 조건·효과 방향은 사용자 첫안을 기록한 것이며 구현 완료가 아니다.
...
Shield는 소이탄·관통 등의 가치를 확인하는 대상이다. 방패 초과 피해·관통 상호작용의 정확한 규칙은 미정이며 임의로 확정하지 않는다.
...
| 중장 방패병 | 강한 개인 방패 + 바로 뒤/주변 일부 일반 적에게 부분 보호. 먼저 제거하면 뒤 Horde 처리 효율 상승. 소이탄/관통/집중화력 판단 검증, 일반 Shield와 즉시 시각 구별 |
...
**Historical boundary:** 아래 장기 방향은 보존하며, M12 Prototype의 실제 해금 범위·조건은 §1.27로 승격/대체했다. 제목의 Future 표기는 M12 밖 확장에만 적용한다.
...
**Historical boundary:** 아래 방향의28기록·숙련 합산·자동 완료·직접 해금은 M12에서 구현 범위로 승격했다. 현재 조건은 §1.27·§3.14를 우선한다.
...
### 1.27 M12 — 작전 기록 / Marine 숙련 / 실제 점진 해금

**§1.20·§1.21의 Future 표기와 과거 개발용 전체 해금은 아래 구현 범위에서 superseded다.** Hades는 점진 해금과 반복 동기의 보조 참고이며 구체 시스템을 복제하지 않는다. “플레이할수록 게임 자체가 계속 커진다”는 경험을 실제 Runtime Pool로 만든다. 첫 Run의 어려움은 적은 Build Option과 아직 구매하지 않은 연구에서 나오며, 공격력 -80%나 Enemy HP 강제 증가 같은 신규 계정 디버프는 추가하지 않는다. 좋은 Gauss Build·RNG·플레이로 첫 Run Boss를 처치할 가능성을 시스템적으로 막지 않는다.

Fresh Save는 Marine / Gauss Rifle, 공용 공격력·공격속도·치명타 확률, 기본 개조 **관통·점사**만 허용한다. 기본 개조 Slot 한도3은 유지하지만 나머지4종은 Pool에 없다. 기존 낮은 확률의 Rare 이상 Run 사거리 카드는 허용한다. Stimpack과 V Ultimate는 기본 기능으로 유지한다. Special Capacity0, 특수무기3종·Relic·Core·Synergy는 잠긴다. 초기 연구는 기본무기 화력·공격속도·성벽 최대 HP3종이다.

첫 **자연 실패 또는 Stage Clear**는 “첫 작전 종료”를 자동 완료하여 숙련1, 특수무기 시스템·수류탄·슬롯1 및 특수 피해/주기 연구를 연다. 수동 Restart·Hub 복귀는 자연 종료가 아니다. 이후 Lv8 이상 일반 Acquisition Category가 뽑히면 해금된 미보유 무기 중 하나를 보여준다. 수류탄만 해금되어 있으면 해당 무기만 후보가 된다. 선택1회 소비·Weapon Lv1·Hard Pity 없음·기존 낮은 weight를 유지한다. 두 번째 획득은 계정 슬롯2 + 첫 무기 보유 + Character Lv14 이상을 모두 요구한다.

**Marine Mastery = 완료한 작전 기록 Point 합계**다. 별도 처치 Mastery XP·XP Bar·Claim 버튼은 없다. 조건을 달성한 순간 완료·Point·직접/Threshold 해금·Save를 처리하며 중복 지급하지 않는다. 생존/진행, 기본무기/특수무기, Build, Stage/고급 도전 네 Category를 사용한다. M12는28기록/총40Point로 주요 Threshold22보다 충분한 여유를 둔다. 일부 원하지 않는 Build 기록을 건너뛰어도 주요 시스템을 열 수 있어야 한다. 직접 해금 기록은 해당 콘텐츠를 원하는 플레이어의 선택 목표다.

| 숙련 Point 또는 직접 조건 | 실제 해금 |
| --- | --- |
| 첫 자연 Run 종료 | 수류탄·특수 시스템·슬롯1·특수 피해/주기 연구 |
| 3 | Relic 시스템 및 장전 장치/충격 탄약/정밀 조준기3종·성벽 피해 감소 연구 |
| 9 | 유도 미사일·XP 연구 |
| 13 | 특수 슬롯2·기본무기 사거리 연구 |
| 17 | 드론 |
| 22 | 기존 Prototype Synergy3종 자동 조건 검사 |
| Stage1 첫 Clear | Core 시스템·전설 증원 병력 |

계정 Special Capacity는0→1→2다. 첫 자연 종료 전에는 숙련13에 도달하더라도 Capacity0을 유지하여 첫 Run에서 특수 시스템을 우회하지 않는다. 무장 확장 Core는 **현재 Run에만 +1, 최대3**이며 무조건3으로 설정하지 않는다. 이후 계정 슬롯이 열려도 Run Core +1을 보존한다. 해금된 미보유 무기가 없으면 빈 추가 획득 Queue를 만들지 않는다. HUD는 Locked / Unlocked-Empty / Equipped를 구분한다.

Relic은 시스템이 잠겼을 때 Elite 처치로 선택 이벤트나 Drop RNG를 발생시키지 않는다. 해금 이후 미보유·해금 Pool에서 기존 방식으로 추첨하며, 추가 유물 직접 해금만 완료해도 Relic 시스템 자체가 열리기 전에는 사용할 수 없다. Core는 Stage1 첫 Clear 이전에는 Roll 자체를 생략하고, 이후 기존 유효3종·약3%·Run최대1을 유지한다. Synergy는 시스템 해금 전 조건을 만족해도 활성화하지 않는다. 잠긴 개조·특수무기·유물·Tree·Overclock은 실제 Pool에서 제외하며 직접 선택 검증도 통과하지 못한다.

| 무기 | 첫 Lv3 Main Tree2종 | 세 번째 Tree 해금 | 첫 Lv20 Overclock2종 | 세 번째 Overclock 해금 |
| --- | --- | --- | --- | --- |
| 수류탄 | Cluster / High Explosive | 수류탄 Lv10 → Tactical | 전술 핵탄두 / 자동 유탄 난사 장치 | 해당 무기 OC 최초 선택 → 삼중 투척 시스템 |
| 미사일 | Saturation / Hunter Killer | 기본 Retarget 누적10회 → Special Tracking | 사냥 본능 폭주 / 전장 사냥망 | 해당 무기 OC 최초 선택 → 불멸 유도체 |
| 드론 | Squadron / Heavy Gunship | 드론 Lv10 → Escort | 무인 전투군 / 전투 순양기 | 해당 무기 OC 최초 선택 → 완전 동기화 |

첫 Lv20은2개만 제시한다. 잠긴 후보로3장을 채우지 않는다. **Lv15 초월은 기존3종을 모두 유지**하며, 전체 목표 OC5종은 Future다. OC 최초 선택 기록은 무기별 숙련2를 지급한다. Lv20 도달 기록과 실제 OC 선택 기록은 서로 다른 조건이다.
...
| 소이탄 | 연소 확산 → **화염 전염**: 사망 시 주변에 화상 전달, Lv10 대상/범위/전달 중첩 강화 | 열축적 → **임계 과열**: 최대 중첩 증가, Lv10 최대 중첩 Tick ×1.6 |

Lv6~9는 위 방향을 강화한다. Lv10은 숫자/행동 차이가 전투에 반영되는 첫 완성이며 추가 카드를 고르는 이벤트가 아니다. 폭발탄은 수류탄의 대형 공간 폭격을 대체할 정도로 범위를 늘리지 않는다. 소이탄은 점사·다중탄과 계속 자유 조합한다. 반복 Level은 피해·효율 숙련으로 이어지고 발수/피해 계산량·최소 주기에는 안전 상한을 둔다.
...
| 기본무기 | 느린 단발 Gauss·초기 Grunt 1-shot 방향, 관통/도탄/점사/다중탄/폭발탄/소이탄 6종, 기본 최대3종, 반복 Level 성장 |
...
### 2.12 M12 Prototype Scope — 실제 Pool·Save·UI 연결

- `operations.ts`에28개 기록과 Prototype 조건·Point·직접 해금·Threshold를 정의하고 완료 ID로 숙련 및 `UnlockState`를 도출한다. UnlockState는 개조·무기·계정 capacity·Relic 시스템/Pool·Core·Synergy·무기별 Tree/OC·연구를 구분한다.
- 실제 Marine Run은 계정 Unlock Snapshot을 전달한다. 기록 달성 후 Unlock State를 갱신하되 열린 Offer는 재생성하지 않는다. 기존 순수 전투/성장 테스트의 인자 생략 전체 접근 기본값은 호환 경로이며 Fresh 플레이 규칙이 아니다.
- Gauss 한 Attack Action의 실제 피해 적중 ID를 중복 없이 세며 점사 여러 발도 같은 행동에 포함한다. 기본무기100Kill 및 Gauss Elite Kill은 실제 원인으로 귀속한다. 특수무기의 피해를 Gauss 처치로 기록하지 않는다. 치명타 기록은 기본/특수무기 실제 HP 또는 방패 피해를 요구하며 발사 RNG 성공·빗나감만으로 완료하지 않는다.
- 미사일 누적 Retarget은 사망/소실된 표적 대신 유효 표적을 실제 재지정하며 재지정 횟수를 소비한 이벤트다. 유효 표적 명중 후 Chain/Phoenix 이동은 제외한다. `totalRetargets`의 Run 절대값을 전달해 중복 이벤트 집계를 막고 계정에는 Run 간 누적한다.
- Run 단위 처치·생존·동시 보유 조건은 한 Run 증거의 최고값으로 판정하고 여러 Run을 합산하지 않는다. Run 시작/Restart에서 활성 증거를 새로 만들며, 이미 달성한 행동 기록은 즉시 저장해 수동 종료 후에도 남긴다. 자연 종료 기록만 실제 실패/Clear를 요구한다.
- Save payload `version: 2`를 사용하며 기존 key `horde-defense:meta:v1`을 유지해 M11 Save를 찾는다. 완료 기록·계정 진행, 활성 Run 증거/이번 완료/해금, 마지막 정산 요약을 검증·저장한다. 숙련과 해금 전체를 별도 수정 가능한 진실값으로 중복 저장하지 않고 기록에서 계산한다.
...
**확정 구현 범위:** 공용 공격력/공격속도/치명 확률3종, 관통/도탄/점사/다중탄/폭발탄/소이탄6종 중 최대3종, 상한 없는 반복 Level, 별도 사거리 카드, 종류와 분리된 희귀도, 투자 가중치, 대성공6%, 대표 전설3종, M3 Owner HUD 연결. 자연 조합은 별도 시너지 카드 없이 동작한다. Marine Magic은 계속 제외하고 자산을 보존한다.
...
| 추적 섬멸망 | 점사+소이탄 / 미사일 헌터B 전술 사냥꾼 / 드론 편대B 울프팩 | 최고 위험 적에 고정 Hunt Mark. Gauss/미사일/드론 집중 및 표적 피해×1.4. 죽거나 사라지면 다음 위험 표적으로1회 전이, 살아 있는 동안 점수 변동으로 왕복하지 않음 |
...
### 3.14 M12 — 기록28개 / 총40Point와 해금 첫안

아래 Point·횟수·Level·시간·Threshold는 **Prototype Tuning**이며 최종 성장 속도 확정이 아니다. 출처는 `src/game/data/operations.ts`다. “완성된 소총”과 요청의 “기본무기 완성 준비”는 동일한3개조 조건이므로 **한 기록/1Point**로 통합한다. 연구 해금을 위한 “치명적 적중”1Point를 명시하여 실제 구현은28기록/40Point다.

| Category | 기록 | 실제 조건 | Point | 직접 보상 |
| --- | --- | --- | --- | --- |
| 생존/진행 | 첫 작전 종료 | 첫 자연 Run 종료 | 1 | 특수 시스템·수류탄·슬롯1·특수 피해/주기 연구 |
| 생존/진행 | 전선 유지 | 한 Run5분 생존 | 1 | — |
| 생존/진행 | 전투 적응 | Character Lv10 | 1 | — |
| 생존/진행 | 첫 정예 격파 | Elite 최초 처치 | 1 | Elite/Boss 피해 연구 |
| 생존/진행 | 버티는 힘 | 한 Run10분 생존 | 1 | — |
| Stage/고급 도전 | 공성 거인 조우 | Stage1 Boss 첫 등장 | 2 | — |
| Stage/고급 도전 | 첫 승리 | Stage1 최초 Clear | 3 | Core·전설 증원 병력 |
| 기본/특수무기 | 관통의 이해 | 한 Gauss Action으로 서로 다른3명 실제 적중 | 1 | 도탄 |
| 기본/특수무기 | 연속 사격 | 점사 Lv3 | 1 | 다중탄 |
| 기본/특수무기 | 점화 실험 (`elite-sniper`) | 한 Run에서 기본무기로 적 처치 | 50 | 소이탄 |
| 기본/특수무기 | 대량 살상 | 한 Run 기본무기100Kill | 1 | 폭발탄 |
| Build | 강력한 한 방 | Unique/Legendary 일반 성장 첫 선택 | 1 | 과충전 축전기 |
| Build | 완성된 소총 | 기본 개조3종 동시 보유 | 1 | 탄약 복제기 |
| 기본/특수무기 | 수류탄 숙련 | 수류탄 Lv10 | 1 | Tactical |
| 기본/특수무기 | 재추적 | 미사일 기본 Retarget 계정 누적10회 | 1 | Special Tracking |
| 기본/특수무기 | 드론 숙련 | 드론 Lv10 | 1 | Escort |
| 기본/특수무기 | 수류탄 오버클록 | 수류탄 OC 첫 선택 | 2 | 삼중 투척 시스템 |
| 기본/특수무기 | 미사일 오버클록 | 미사일 OC 첫 선택 | 2 | 불멸 유도체 |
| 기본/특수무기 | 드론 오버클록 | 드론 OC 첫 선택 | 2 | 완전 동기화 |
| Build | 두 개의 무장 | 특수무기2종 동시 보유 | 1 | — |
| Build | 첫 완성 | 어떤 특수무기든 Lv10 | 2 | — |
| Build | 첫 초월 | 어떤 특수무기든 Lv15 | 2 | — |
| Build | 한계 돌파 | 어떤 특수무기든 Lv20 | 3 | — |
| Build | 첫 유물 | Relic 첫 획득 | 1 | — |
| Build | 유물 완성 | 한 Run Relic2개 보유 | 1 | — |
| Build | 첫 시너지 | Synergy 첫 활성화 | 2 | — |
| Build | 규칙 파괴 | Core 첫 획득 | 2 | — |
| 기본/특수무기 | 치명적 적중 | 기본/특수 치명타 실제 피해 | 1 | 치명 확률/피해 연구 |

Gold 연구 공개는 효과 수식과 분리한다. 초기3종(기본 피해·공속·성벽 HP), 첫 작전 종료2종(특수 피해·주기), 첫 치명타2종(치명 확률·피해), 첫 Elite1종(Elite/Boss 피해), 숙련3 방어·9 XP·13 사거리 순서다. 잠긴 연구는 구매할 수 없고 기존 구매 연구는 Migration 후 효과와 접근을 유지한다. M11의11종 Level20/10/5·돌파·비용표와 Reroll50/150/400은 변경하지 않는다.
...
| 소이탄 | Tick 500ms, 지속 3000ms, 최대2중첩, Stack당 Tick 계수 `0.025 + 0.010×품질`. /dev Runtime 노출 |
...
직접 해금 후보: 관통 다중 처치→도탄, 점사 성장→다중탄, 한 Run 기본무기 50처치→소이탄, 수류탄 누적 처치→전술탄, 수류탄 특정 Lv10→새 Overclock, 미사일 Lv20 최초→미사일 Overclock, 드론 요격→요격 성장. 정확한 조건·숫자는 미정이다.
...
**플레이 조건/작전 기록 + 필요하면 Credits** 형태를 우선한다. 예: 도탄 기록 해금, 다중탄 숙련+Credits, 소이탄 기본무기 50처치 기록, 고급 유물 발견+Credits, 내부 Variant/Overclock 사용기록+Credits. 수류탄/미사일/드론 자체를 Credits Grind만으로 잠그지 않는다.
...
다음은 역사적 변경표다. 마지막 문장의 v0.15 우선 표기는 당시 기록이며 현재 상충 시 v0.17의 소이탄 규칙을 우선한다.
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. M11 Credits 연구로 Run당 최대3회 해금한다. v0.4/v0.5를 포함한 과거 버전은 역사 문서로 보존하며 상충 시 최신 v0.17을 우선한다.
...
| 영역 | M12 구현 범위 / 남은 확인 |
| --- | --- |
| Fresh/실제 Pool | Gauss·관통/점사·공용3종·Rare Range·Stimpack/V, Special0. 잠긴 Mod/Special/Relic/Core/Synergy/Tree/OC 실제 차단, 열린 Offer 유지 |
| 작전 기록/숙련 | 28기록/40Point·자동 저장·중복 완료 방지·Threshold3/9/13/17/22·직접 해금. 런별 행동 증거와 계정 미사일 재지정 누적 구분 |
| 무기/슬롯 | 자연 첫 종료 수류탄/슬롯1, 슬롯2 숙련13, Core 현재 Run+1/최대3. Lv8/14 Acquisition·RNG·선택 소비 유지 |
| 무기 내부 성장 | 최초 Tree2종/OC2종, 기록으로 세 번째 해금, Lv15는3종 유지. 후보 부족 시 실제2장만 표시 |
...
### 5.6 Historical M11 Implementation Gap

다음은 v0.15 당시의 구현·검증 기록을 그대로 승계한다. M11이 아직 미병합이라는 상태 및 M12 콘텐츠 제외 문구는 현재 상태가 아니다.
...
# v0.17 — 소이탄 기본무기 개조 교체

v0.16 전체 설계를 승계하며 원본 문서는 보존한다. 이 절은 상충하는 과거 설명보다 우선한다. `heavy` 고위력 단발의 피해/주기 Mechanic은 폐기하고 여섯 번째 개조를 `incendiary` 소이탄으로 교체한다. 다른 5개 개조, 특수무기, 적, Meta 수치는 유지한다.

## Gauss 화상 / 중첩

Lv1부터 Gauss로 실제 피해를 받은 Enemy에 화상을 부여한다. 한 Round에 같은 Enemy는 최대 1 Stack만 추가하며 Direct/Pierce/Ricochet/Multishot/Explosion 중복 피해도 공유한다. Burst의 다음 Round는 다시 추가한다. Grenade/Missile/Drone 자체는 점화하지 않는다. 관통·도탄은 맞은 적, 다중탄은 유효 보조탄 대상, 폭발탄은 실제 Splash 피해 대상까지 점화하므로 다른 5종과 자유 조합한다.

Prototype 기본값은 Tick 500ms, 지속 3000ms, 최대 2 Stack, Stack당 Tick 피해율 `0.025 + quality × 0.010`이다. Common Lv1은 0.035, 6 Tick 합계 약21%의 지연 피해다. 재적중은 상한 미만이면 Stack +1하고 상한에서도 지속시간을 갱신한다. 더 강한 화상을 약한 화상으로 덮어쓰지 않는다.

화상 피해는 기본 Gauss 공격력/Meta/Elite·Boss 보너스/유효 Target multiplier와 합성한다. Tick은 치명타·관통·도탄·폭발·다중탄·Ammo Duplicator·Gauss on-hit을 생성하지 않는 damage-only 경로다. Kill/XP/기본무기 처치/Operation/보스 및 드롭 처리는 공통 처치 흐름을 사용하며 Telemetry는 Gauss와 Total Damage에 포함한다.

## Lv5 / Lv10 / 숙련

| 분기 | Lv5 | Lv6~9 | Lv10 완성 |
| --- | --- | --- | --- |
| A 연소 확산 | 화상 적 사망 시 근처 2명, 반경90, 1 Stack, Stack 피해×0.70 전달 | 대상 수·반경·전달 효율을 선형 성장 | 화염 전염: 4명, 반경140, 2 Stack, 전달×0.90 |
| B 열축적 | 최대4 Stack | 최대 중첩과 Tick 효율 완만히 성장 | 임계 과열: 최대6 Stack, 효율+15%, 최대 중첩 Tick×1.6 |

A는 화상 처치도 이후 전파 가능하되 동일 순간 재귀 연쇄 피해를 생성하지 않는다. 후속 Tick/Death의 자연스러운 연쇄를 허용한다. B는 Elite/Boss에 별도 패널티 없이 적용한다. Lv11+ 품질 숙련 성장은 계속되고 Level cap/추가 분기는 없다. 희귀도와 A/B는 독립이며 소이탄 전용 Legendary Mechanic은 Future다. Legendary 카드는 기존 품질 증가만 적용한다.

## 해금 / 호환 / Synergy

Fresh Account는 관통·점사로 시작한다. `elite-sniper` 내부 Operation id를 유지하고 표시를 **점화 실험**, 조건을 한 Run 기본무기 50처치(`primaryKills`), 보상을 소이탄 해금으로 변경한다. 기존 completed 기록은 즉시 소이탄 해금으로 승계한다. 폭발탄의 기본무기 100처치는 유지한다. 영구 Save version 증가와 현재 Run migration은 필요 없다.

추적 섬멸망 조건은 **점사 + 소이탄 + 전술 사냥꾼 + Wolfpack**이다. 점사로 중첩을 쌓고 위험 표적을 집중 제거하며 기존 표적 피해×1.4와 전이 규칙을 유지한다. 미구현 시너지의 소이탄 표기는 조건 후보의 ID 교체일 뿐 신규 동작 구현을 의미하지 않는다.

## /dev / 시간 / 표현

Quick 소이탄 카드는 획득 가중치0.80, 성장 가중치1.00, 기본 Tick 피해율, 지속시간, 기본 최대 중첩의 5개 조정값이다. Detail은 Tick 간격/품질 성장/A의 Lv5·Lv10 대상·반경·전달 Stack·전달 피해율/B 중첩·효율·과열 배율을 포함한 실제 Runtime key를 공유한다.

Legacy `marineModWeights.heavy.*`의 알려진 두 가중치는 소이탄으로 승계하며 명시적인 새 key가 우선한다. 알려진 heavy 피해·주기 key 4개만 제거하고 다른 알 수 없는 key 검증은 유지한다. 화상은 Enemy id 기준으로 현재 생존 적 수에 bounded되고 사망/삭제 시 제거한다. Combat simulation time으로 갱신하므로 Pause/Level-Up/X1~X8의 의미가 같으며 wall-clock/setTimeout/무한 history는 쓰지 않는다. 상태 표시는 기존 렌더링에 따뜻한 tint/작은 marker 수준으로 추가하고 Final Art나 적별 고비용 Object는 만들지 않는다.
