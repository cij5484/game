...
**Current Source of Truth — Independent Screen Regions / Build Ownership HUD**

작성일: 2026-09-18. **목표 설계 문서이며 구현 완료 보고가 아니다.**
기준: [GDD v0.6](GAME_GDD_v0.6.md) 전체와 이후 확정된 화면 구조·Build HUD 설계. v0.6은 수정하지 않는 historical record다. M2 gameplay baseline은 `a656b122e97850a086bf8a3ca0f74dc15a3b1a5b`이며 PR #17 / main `9df9878`에 반영됐다. M3의 실제 구현 범위는 [구현 기록](../prototype-m3-layout-build-hud.md)을 따른다.

상충 시 v0.7을 우선한다. v0.1~v0.6, Technical Spec, 과거 계획·완료 기록은 당시의 설계/구현 기록이다. 이번 작업은 화면 배치·Build 표현·첫 교전 Tempo와 추가 요청된 몹 이동속도1.5배를 반영한다. 새로운 성장/특수무기/보상/Meta를 구현하는 패스가 아니다.
...
M3는 두 슬롯을 Locked로 보여줄 뿐 실제 계정 해금/특수무기를 만들지 않는다. 향후 무장 확장 Core의 2→3 슬롯은 동적 추가 가능하게 하되 M3 기본 표시는 2개이며 Core/세 번째 슬롯 획득은 Future다.
...
- **특수무기 슬롯:** 해당 무기 Level / Lv3 Main Tree / Lv6 Sub Branch / Lv10 Completion / Lv15 Transcendence / Lv20 Overclock / 기타 전용 성장. Grenade는 Grenade, Missile은 Missile, Drone은 Drone 슬롯에 묶는다. 실제 내용 연결은 Future다.
- **스팀팩 슬롯:** 고유 성장/분기 표시 가능. M3는 존재하는 Legacy 스팀팩 성장을 연결한다.
...
M3는 Range .55와800ms를 유지하고 초기24명 중3명만 progress .43~.445에 배치한다. 나머지는 기존 먼 배치를 유지한다. 추가 플레이 피드백에 따라 일반/정예 이동속도 곡선은 M2의1.5배(생성 시 배율 .65→.975, 최종1.02→1.53)로 조정한다. HP·공격 주기·돌진 예고 시간은 유지한다. 이는 Prototype Tuning이며 새로운 Enemy/Director 규칙 확정이 아니다.
...
**아래는 1차 Prototype 전체 목표이며 M3 구현 완료 목록이 아니다.** 장기 전체 Pool과 실제 첫 구현 Pool을 구분한다. Marine 고정, Stage 1 하나로 핵심 재미를 검증한다.
...
아래는 **첫 시험 목표이며 최종 확정 수치가 아니다.** Future Economy 숫자는 설계 기록이며 1차 Prototype 구현 의무가 아니다. 아래 장기 수치는 M3 데이터/테스트 기대값에 적용하지 않는다. M3 화면 및 초기 배치 Tuning은 별도 기록한다.
...
기본 개조6종·3슬롯, 무한 공용/숙련 성장, 독립 희귀도·전설, 특수무기 트리 및 전체 Overclock5종, 대성공, 유물 무레벨/무패널티, High-Variance, Portrait, 스팀팩·고유 Gesture, Reroll 기본0 방향은 유지한다. v0.4/v0.5는 역사 문서로 보존하며 상충 시 최신 v0.7을 우선한다.

## 5. Current Implementation Gap

현재 gameplay baseline: M1 `d15b41d` → M2 `a656b12` (main merge `9df9878`). M3는 그 위에서 Layout/Build HUD/첫 교전 배치를 변경하고 추가 사용자 요청에 따라 몹 이동속도를1.5배로 조정한다. 검증 근거와 구현 파일은 [M1](../prototype-m1-combat-foundation.md), [M2](../prototype-m2-stage1-core.md), [M3](../prototype-m3-layout-build-hud.md)에 분리 기록한다. 역사적 `0489ef9` 검증 기록을 최신 결과로 바꾸지 않는다.

| 영역 | 현재 실제 상태 / 남은 Gap |
| --- | --- |
| M1 전투 | Range progress≥.55,800ms 단발 피해10,Grunt HP8,집중 타겟. v0.7 목표 중거리의 Prototype 값 |
| M2 적/Director | 방패HP→본체HP,두 Elite 행동,20분 Director/완화/후반 cap180. Boss 없이20분 종료는 임시 처리 |
| Marine Loadout | Legacy Magic 코드 보존, Marine에서는 사용/표시/성장/전투 hooks 분리. 스팀팩/V 유지 |
| M3 화면 | Header/전장/하단 독립 rect,전투 mask,Level/XP/시간 상단,Wall HP/무장 하단 |
| M3 Build | Legacy trait/Hyper Gauss는 기본무기,Stim 성장은 스팀팩,공용·유물·코어·시너지는 Global. UI 분류가 최종 gameplay 구현을 뜻하지 않음 |
| M3 첫 교전 | 선두3명 위치 조정과 사용자 요청 이동속도1.5배.3~5초 목표 실제 simulation 확인.사거리/공격 주기 유지 |
| 기본무기6종 | Legacy 관통/도탄/다중탄/폭발탄/처형탄·MAX5/Lv3분기 존재.최종 점사/고위력/무한 성장/전설/사거리 카드 미구현 |
| 공용/희귀도/대성공 | Legacy 유한 성장/선택/가중치 유지.무한 성장·독립 희귀도·Great Success·특별 이벤트 Queue 미구현 |
| 특수무기/성장 | 두 Locked 슬롯만 M3 구현.실제3종/획득/Lv3·6·10·15·20/세 번째 슬롯 Core 미구현 |
| 유물/코어 | Legacy level/보유 한도/drop 유지.최종 무레벨 유물/교체·신규 Core/품질 소급 미구현 |
| 시너지 | Legacy 선택형만 표현.최종3중 조건 자동 활성 및 신규6종 미구현 |
| Boss/보상 | 공성 거인/기믹/Stage 종료/Boss 보상·새 Elite 보상 확률 미구현 |
| Future | Gold/Credits/기록/숙련/점진 해금/Reroll Meta/희귀 재화/Challenge/Endless/Stage2+/새 Character/Awakening/친구 기록 미구현 |
| 전체 콘텐츠/상세 UX | 전체Relic10/Core5/Synergy6·모든초월/Overclock·Final Art·완성Tutorial·무기별 최종Tooltip 등 후속 |

### 5.1 v0.6 대비 명시적 대체 / 보존

- Header/Battlefield 중첩을 독립 공간으로 대체. progress는 Battlefield 내부에만 매핑.
- Build 전체 Header 나열을 Owner별 Header/Loadout 배치로 대체. 기본무기/특수무기 성장의 중복 표시 금지.
- XP/Level/시간은 Header,Wall HP/Loadout은 Bottom. 잠긴 특수 슬롯2개는 빈 슬롯 숨김 원칙의 명시적 예외.
- 첫 교전3~5초는 Prototype Tuning. 세 영역 높이/간격·clip 범위·시각 표현도 Tuning.
- Stage1/성장/특수무기/Meta 전체 목표는 v0.6에서 계승. M3에서 별도 gameplay 시스템을 추가하지 않는다.
- v0.6 원본은 historical record로 보존한다. M3는 별도 branch commit/push까지만, main merge 및 다음 Milestone은 별도 요청을 따른다.
