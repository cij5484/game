...
선택·수동 일시정지는 전장·공격·spawn·마법/스팀팩·리듬·VFX 시계를 함께 멈춥니다. safe-area와 `viewport-fit=cover`, 터치 스크롤/선택 억제는 유지합니다. 실제 Android/iPhone의 노치·다중 터치·180마리 FPS·가독성은 별도 플레이테스트 대상입니다. HUD 참고 근거는 [REFERENCE_UI_NOTES.md](docs/design/REFERENCE_UI_NOTES.md)에 기록합니다.
...
현재 변경은 codex/milestone-12-combat-depth에서 계속하며 main에는 merge하지 않습니다. 실제 재미/문구/모바일 입력/유물·특성 MAX 도달성/후반180마리 성능은 사용자 Playtest로 평가하고 이후 merge 여부를 결정합니다. 다음 Milestone은 시작하지 않습니다.
