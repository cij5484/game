...
모바일 웹 호드 디펜스의 그레이박스 프로토타입입니다. 현재는 **Milestone 7: Horde Director + Clean Combat HUD**까지 구현했습니다. 세로 전장에서 적의 성벽 공격, Marine의 탭 기반 3점사와 두 손가락 탭 Stimpack을 플레이할 수 있습니다.
...
- 기본 HUD는 얇은 Wall HP와 활성 Stimpack 상태만 표시합니다. 개발 서버에서 **D** 키로 Lane/FAR/NEAR, 조작 설명, 종류/HP/progress, Stim 상태, director 시간/단계/간격/batch/active 수를 표시합니다. 기본 OFF이며 모바일 버튼과 프로덕션 토글은 없습니다.
- 적을 처치하지 않으면 성벽에 쌓여 active 상한에 도달합니다. 빈자리는 처치 또는 다음 단계의 상한 증가 때 생깁니다. 적 충돌/군중 분산과 최종 전투 난이도 조정은 이번 범위가 아닙니다.
- Milestone 7 브라우저 관찰(390×844, 약 3분 20초): 65초 28/36 → 128초 48/48 → 193초 60/60, 콘솔 오류/경고 없음. 성벽 앞 중첩 때문에 실제 수보다 적어 보이는 한계가 있습니다. 실제 모바일 FPS/터치 성능 측정이나 100~200마리 stress test는 수행하지 않았습니다.
