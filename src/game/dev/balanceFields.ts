...
field(
  ...
  "미사일 비행 속도",
  "특수무기 · 미사일",
  "새 미사일의 논리 이동속도입니다. 높이면 표적에 빨리 도달하며 현재 비행 중인 탄은 유지될 수 있습니다.",
  "다음 공격부터",
  10,
  3000,
  10,
...
for (let index = 0; index < 3; index++)
  field(
    `special.missileBehavior.saturation.count.${index}`,
    `포화 미사일 수 · Lv${[3, 6, 10][index]}`,
    "특수무기 · 미사일",
    "포화 계열이 한 번에 발사하는 미사일 수입니다. 높이면 여러 표적을 더 많이 공격합니다.",
    "다음 공격부터",
    1,
    30,
    1,
...
for (const [id, label] of [
  ...
  field(
    ...
    `미사일 · ${label}`,
    "특수무기 · 미사일",
    "자동 표적 점수에 더하는 값입니다. 높이면 해당 위험 표적을 더 우선하며 수동 집중 조준은 유지합니다.",
    "다음 공격부터",
    0,
    5000,
    10,
