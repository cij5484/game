...
it("routes completed single-finger drawings to magic effects and denies cooldown reuse", () => {
  ...
  const z = [{ x: 20, y: 20 }, { x: 160, y: 20 }, { x: 20, y: 160 }, { x: 160, y: 160 }];
  const magic = new Magic();
  for (const [points, expectedId] of [[circle, "frost-nova"], [z, "chain-lightning"]] as const) {
    ...
    if (result?.kind !== "gesture") throw new Error("Drawing was not routed as a gesture");
    const recognized = recognizeGesture(result.points);
    const id = recognized.kind === "circle" ? "frost-nova" : recognized.kind === "z" ? "chain-lightning" : null;
    ...
    if (id === "frost-nova") expect(cast.enemies[0]!.frozenMs).toBeGreaterThan(0);
